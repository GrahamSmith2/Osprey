...
%% T13 -- Nomoto fit reproduces the nonlinear step response
NM = identifyNomoto(P, [5 8 11], deg2rad(5));
[n_pass, n_fail] = ck('T13a Nomoto fit within 10% of the nonlinear response', ...
    all(NM.fit_rms < 0.10), n_pass, n_fail);
[n_pass, n_fail] = ck('T13b K'' constant across speed (K ~ U scaling holds)', ...
    NM.K_prime_spread < 0.35, n_pass, n_fail);
[n_pass, n_fail] = ck('T13c T'' constant across speed (T ~ 1/U scaling holds)', ...
    NM.T_prime_spread < 0.35, n_pass, n_fail);

%% T14 -- gain schedule reproduces the derived scaling exponents
Gs = gainSchedule([4 8 16], NM, P);
[n_pass, n_fail] = ck('T14a angle-domain Kp_r scales as 1/U^2', ...
    max(abs(Gs.Kp_r .* Gs.U.^2 / (Gs.Kp_r(1)*Gs.U(1)^2) - 1)) < 1e-9, n_pass, n_fail);
[n_pass, n_fail] = ck('T14b moment-domain Kp_N is speed-invariant', ...
    max(abs(Gs.Kp_N/Gs.Kp_N(1) - 1)) < 1e-9, n_pass, n_fail);
[n_pass, n_fail] = ck('T14c single-loop Kp scales as 1/U', ...
    max(abs(Gs.single_Kp .* Gs.U / (Gs.single_Kp(1)*Gs.U(1)) - 1)) < 1e-9, n_pass, n_fail);
[n_pass, n_fail] = ck('T14d inner crossover respects the delay ceiling', ...
    Gs.wc_r <= 0.8*Gs.wc_r_ceiling + 1e-9, n_pass, n_fail);

%% T15 -- allocator hands over to differential thrust as speed falls
A_hi = allocate(20, 50, 12, P);
A_lo = allocate(20, 50,  1, P);
[n_pass, n_fail] = ck('T15a at speed the rudder carries the yaw moment', ...
    abs(A_hi.N_rud_cmd) > abs(A_hi.N_thr_cmd), n_pass, n_fail);
[n_pass, n_fail] = ck('T15b at low speed differential thrust takes over', ...
    abs(A_lo.N_thr_cmd) > abs(A_lo.N_rud_cmd), n_pass, n_fail);
[n_pass, n_fail] = ck('T15c deflection never commanded past ventilation onset', ...
    abs(A_hi.delta_cmd) <= P.R.delta_vent_on + 1e-12, n_pass, n_fail);

%% T16 -- closed loop actually converges
fh16 = makeAutopilot(P, NM, struct('wpts',[], 'psi_ref',deg2rad(30), ...
                                   'U_ref',8, 'seed',5));
Tt16 = hullSteadyState(8, P).R_total/2;
S16 = simOsprey([8;0;0;0;0;0;Tt16;Tt16;0], 40, fh16, P, struct('dt',0.002));
psi_end = rad2deg(S16.psi(end));
% Target is 30 deg MEASURED; the 2 deg magnetometer bias means true heading
% settles near 28 deg. The controller cannot see its own sensor bias -- that is
% correct behaviour, not an error.
[n_pass, n_fail] = ck('T16a heading converges to target minus the mag bias', ...
    abs(psi_end - 28) < 2.0, n_pass, n_fail);
[n_pass, n_fail] = ck('T16b no ventilation events with the onset clamp active', ...
    sum(diff(S16.vent) > 0) == 0, n_pass, n_fail);
[n_pass, n_fail] = ck('T16c yaw rate stays inside the roll envelope', ...
    max(abs(S16.r)) < rollEnvelope(8, P).r_max_safe, n_pass, n_fail);
